// Per-order social meta for the public proof page. Scrapers do not run JS, so the SPA's
// site-wide tags are rewritten here into order-specific title/description/image before the
// HTML is returned. Humans still get index.html and the SPA boots and routes client-side.
//
// The og:image points at the dynamic card (/og/order/:id.png). The proof fetch only enriches
// the description text and is best-effort with a short timeout, so the page never blocks on it.
