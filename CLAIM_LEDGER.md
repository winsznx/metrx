...
| Settlement contract enforces PAY / REFUND / SLASH from an EIP-712 verdict | Proven | Mainnet 677 | [`0x8b6079…15532`](https://scan.botchain.ai/address/0x8b607937eE86Bfc9de57F5d2F8E9d02F58415532) | 52 Foundry tests plus three settled mainnet orders | Verdict correctness is trusted, not proven | Live |
...
| The verifier refuses to sign on malformed output, self-contradiction, out-of-range score, model mismatch, missing key, or rate limit | Tested | — | — | 10 tests in `workers/api/test/groq.test.ts` | Refusal paths are stubbed; the happy path is proven by three mainnet settlements | Ready |
...
| A rubric can be tested before any money moves | Proven | — | `POST /api/preview` | Runs the real verifier with no order, no chain write and no signature; 2 tests | Uses the same shared model quota, so it is rate limited | Live |
| A buyer can see the supply side before funding | Proven | Mainnet 677 | `GET /api/operators` | Registered operators and largest unlocked stake, read from `OperatorRegistered` logs; 1 test | Reports stake, not willingness — an operator may still decline | Live |
| A user can always find their own orders | Proven | Mainnet 677 | `GET /api/orders?address=` | Address index from `OrderCreated` and `OrderAccepted` logs; 1 test | Falls back to a full scan if the deploy block is misconfigured | Live |
| A signed certificate cannot be lost or re-spent | Proven | Mainnet 677 | `/proof/1`, `/proof/2`, `/proof/3` | Cached per committed output hash and recovered from settlement calldata; 2 tests | Cache is per deployment; calldata recovery is the durable path | Live |
| Model quota cannot be drained by an anonymous caller | Tested | — | — | Per-IP limit on `/api/verify` and `/api/preview`, plus certificate caching | Coarse per-IP bucket, not per-account | Ready |
...
| MetrxCore is deployed to BOT Chain Mainnet | Proven | Mainnet 677 | [`0x8b6079…15532`](https://scan.botchain.ai/address/0x8b607937eE86Bfc9de57F5d2F8E9d02F58415532) · [deploy tx](https://scan.botchain.ai/tx/0x9cb0c561e32601a3b2de2a976b25c92966945b59706daefcaf37064319ba38ee) | 10,739 bytes of code, deployed at block 20153854 | No admin, no upgrade path | Live |
