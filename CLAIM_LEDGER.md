...
| Settlement contract enforces PAY / REFUND / SLASH from an EIP-712 verdict | Proven | Mainnet 677 | [`0x868ee0…5B018`](https://scan.botchain.ai/address/0x868ee03536A046DcFa568BbaE29C1C3a9f85B018) | 50 Foundry tests plus two settled mainnet orders | Verdict correctness is trusted, not proven | Live |
...
| The full loop works through the real API routes: fund → deliver → verify → settle → proof | Proven | Mainnet 677 | orders #1 and #2 | `api.e2e.test.ts` on anvil plus two live mainnet lifecycles | Test run uses mock mode; mainnet used Groq | Live |
...
| Verdicts are schema-enforced by the model, not repaired after the fact | Proven | Mainnet 677 | orders #1 and #2 | Strict `json_schema` asserted in `groq.test.ts`; both mainnet verdicts returned schema-conformant | Only on `openai/gpt-oss-*`; other models fall back to `json_object` | Live |
| Published artifacts still reproduce their on-chain hashes after settlement | Proven | Mainnet 677 | `/api/proof/1`, `/api/proof/2` | 7/7 hash checks match on both orders | Artifact availability is not chain-guaranteed | Live |
...
| MetrxCore is deployed to BOT Chain Mainnet | Proven | Mainnet 677 | [`0x868ee0…5B018`](https://scan.botchain.ai/address/0x868ee03536A046DcFa568BbaE29C1C3a9f85B018) · [deploy tx](https://scan.botchain.ai/tx/0x3eaf5811797a8b739e2897175fa4937c3de0df722370e5eede82dd7068f1b196) | 10,521 bytes of code at block 19930429 | No admin, no upgrade path | Live |
| Source verified on BOTScan | Proven | Mainnet 677 | [#code](https://scan.botchain.ai/address/0x868ee03536A046DcFa568BbaE29C1C3a9f85B018#code) | Blockscout reports `Pass - Verified`, solc 0.8.28, 200 runs | — | Live |
| One completed PAY lifecycle on mainnet | Proven | Mainnet 677 | [settle tx](https://scan.botchain.ai/tx/0xc4ed4b8476ecd5d82d28a2bd63092e0d66514fde51355fb9780484caf6b4afd9) | Order #1 is `Paid`, 0.01 BOT to the operator, PASS at 100% | Single job, bounded text task | Live |
| One completed REFUND/SLASH lifecycle on mainnet | Proven | Mainnet 677 | [settle tx](https://scan.botchain.ai/tx/0x540d789e2018f1226773579224f88fa591c67ba95c9fba152dcfbfbbb7a6405b) | Order #2 is `Slashed`, buyer got 0.015 BOT, FAIL at 0% on all 3 rubric items | Single job, bounded text task | Live |
...
| Web app is publicly reachable and wired to the live contract | Proven | Cloudflare Pages | https://metrx.pages.dev | HTTP 200 on `/`, `/proof`, `/docs/what-is-real`; proof hub reads orders #1 and #2 | — | Live |
